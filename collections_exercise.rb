...
# for easier access to the actual content
articles_array = articles[:response][:results]

# iterate through the items and add a new key value pair to each article
articles_array.each do |article|
  article[:views] = 0
end

# taking a sample of the list of articles and add the view by 1 each time it is accessed
